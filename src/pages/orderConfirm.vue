<template>
  <div class="order-confirm">
    ...
    >
      <!-- 将所有的数据通过v-modal绑定为一个对象的各个属性的数据，这样用post上传表单时只用传参该对象即可 -->
      <template v-slot:body>
        <div class="edit-wrap">
          <div class="item">
            <!-- placeholder:默认显示文本 -->
          ...
          <div class="item">
            ...
            <select name="district" v-model="checkedItem.receiverDistrict">
              <!-- value不赋值，选中该选项也不会给v-model选中的数据赋值 -->
...
export default {
    ...
    data(){
      return{
        ...
        showEditModal:false, //是否展示编辑弹框
        checkedIndex:0, //当前收获地址选中的索引
      }
    },
    mounted(){
      this.getAddressList();
    ...
    methods:{
      ...
      getCartList(){
        this.axios.get('/carts').then((res)=>{
          ...
          this.cartList = list.filter(item=>item.productSelected); //过滤出已经选中的商品
